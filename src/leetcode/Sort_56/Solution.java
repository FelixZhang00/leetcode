 ...
 * Runtime:387ms
 *
 * 先对intervals集合按start从小到大排序，last变量用于保存可能插入到结果集中的元素，
 * 遍历每一个集合中的元素，如果符合合并的条件，将last和当前元素合并,并重新赋值给last，
 * 此时last仍然具有合并的潜力；如果不符合合并的条件，则将last放入结果集中，并把当前元素赋值给last，成为一个新的潜在具有合并性的元素。
 */
