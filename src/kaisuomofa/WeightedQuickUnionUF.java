...
public class WeightedQuickUnionUF {
    ...
    private int index = 0;    //以根节点作为索引
    private HashMap<Integer, Integer> nums;     //每个连通分量内的个数
    ...
    public void union(int p, int q) {
        ...
        if (size[rootP] < size[rootQ]) {
            ...
            changeComponent(rootQ, rootP);
        } else {
            parent[rootQ] = rootP;
            size[rootP] += size[rootQ];

            changeComponent(rootP, rootQ);
        }
        count--;
    }
     ...
     * @param oldRoot
     */
    private void changeComponent(int root, int oldRoot) {
        nums.put(root, size[root]);
        if (nums.containsKey(oldRoot)) {
            nums.remove(oldRoot);
        }
    }
...
//        Scanner scan = new Scanner(args[0]);
        ...
        HashMap<Integer, Integer> nums = uf.nums;
        for (Map.Entry<Integer,Integer> set: nums.entrySet()){
            int num=set.getValue();
            System.out.println("每个连通分量内含有元素："+num);
        }
    }
